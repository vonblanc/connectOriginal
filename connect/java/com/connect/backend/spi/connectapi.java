...
import com.connect.backend.domian.utilities.Inbox;
import com.connect.backend.domian.utilities.TextMessage;
...
public class connectapi{
	...
	public Profile getProfile(@Named("webSafeProfileKey")String webSafeProfileKey) throws NotFoundException
	{
			Key<Profile> profileKey=Key.create(webSafeProfileKey);
			
		    Profile profile=ofy().load().key(profileKey).now();
		    		    
		    if(profile==null) throw new NotFoundException("Can't find Profile");
	...
	public ServiceReq createServiceReq(ServiceReqForms serviceReqForms, @Named("webSafeServicePeopleKeys")List<String> webSafeServicePeopleKeys, @Named("webSafeClientKey") String webSafeClientKey) throws NotFoundException
	{
		...
		ServiceReq serviceReq= new ServiceReq(ServiceReqKey.getId(),clientKey.getString(),serviceReqForms,webSafeServicePeopleKeys);
    ...
    @ApiMethod(name="createServiceResp",path="createServiceResp",httpMethod=HttpMethod.POST)
    
   public ServiceResp createServiceResp(ServiceRespForms serviceRespForms, @Named("seProfileKey")String seProfileKey,@Named("serviceRequestKey") String serviceRequestKey) throws NotFoundException, ForbiddenException
   {
    	Key<Profile> profileKey=Key.create(seProfileKey);
    	
    	if(profileKey==null) throw new NotFoundException("Profile not found");
    	
    	Key<ServiceReq> requestKey=Key.create(serviceRequestKey);          //create service request Key from service request String 
    	    	
    	ServiceReq serviceReq=ofy().load().key(requestKey).now();                               //load service request
        
    	if(serviceReq==null) {throw new NotFoundException("No such service request");}                  //check if service request exist 

    	
    	
		Key<ServiceResp>respKey= ObjectifyService.factory().allocateId(requestKey,ServiceResp.class);  //creates key from the user key and client class(id also created)
    	
		long respId=respKey.getId();
		ServiceResp serviceResp= new ServiceResp(respId,serviceReq,profileKey.getString(),serviceRespForms);
		
		ofy().save().entity(serviceResp);
	...
	public List<Profile> queryProfilesByJobTag(@Named("tags")String search)
	{
	...
	@ApiMethod(name="testInbox",path="testInbox",httpMethod=HttpMethod.GET)
	public void testqueries(@Named("message") String message)
	{
		TextMessage tMessage=new TextMessage(message);
		
        //inbox.addMessage(tMessage);
	...
	public Profile createTestProfiles(AboutMeForms aboutmeforms) throws ParseException, NotFoundException
	{
		//AboutMe aboutme=new AboutMe(aboutmeforms);
		final String webSafeConnectUserKey="agkke2FwcC5pZH1yLQsSC0Nvbm5lY3RVc2VyIhwwR0x6REN0SllpZFhQOWZld0pVMnNKRHpieW8xDA";
